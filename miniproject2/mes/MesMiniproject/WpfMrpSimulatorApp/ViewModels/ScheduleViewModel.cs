    ...
    public partial class ScheduleViewModel : ObservableObject
    {
        ...
        public async Task SaveData()
        {
            ...
            try
            {
                ...
                using (var db = new IoTDbContext())
                {
                    if (schedlue.SchIdx == 0)   // 신규
                    {
                        schedlue.RegDt = DateTime.Now; // 등록일 현재일자
                        db.Schedules.Add(schedlue); // ASP.NET Core에서 한 작업과 동일
                    }
                    else  // 수정
                    {
                        var origin = db.Schedules.Find(schedlue.SchIdx);    // ASP.NET Core와 동일
                        if (origin!= null)
                        {
                            origin.PlantCode = schedlue.PlantCode;
                            origin.SchDate = schedlue.SchDate;
                            origin.LoadTime = schedlue.LoadTime;
                            origin.SchStartTime = schedlue.SchStartTime;
                            origin.SchEndTime = schedlue.SchEndTime;
                            origin.SchFacilityId = schedlue.SchFacilityId;
                            origin.SchAmount = schedlue.SchAmount;
                            origin.ModDt = DateTime.Now;
                        }
                    }
                    db.SaveChanges(); // COMMIT
                    await this.dialogCoordinator.ShowMessageAsync(this, "공종계획 저장", "데이터가 저장되었습니다.");
                }
            }
            catch (Exception ex)
            {
